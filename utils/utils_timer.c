...
static tmr_entry *tmr_node(int id)
{
	tmr_entry *node = NULL;
	POSITION pos;
	list_lock();
...
// this interface is different than TMR_add_repeat, it will modify the fired time,
int TMP_feed_timer( int id, tmr_hander handle_fxc, int t_period, int count )
{
	int ret = tmr_find(id);
	if ( ret ==0 && t_period > 0 && handle_fxc )
	{
		tmr_entry *entry = malloc(sizeof(tmr_entry));
		if ( entry==NULL )  // out of heap memory
			return -1;
		...
		if( ret == 1 )
		{
			// if we find the timer id, we modify it , and re set timeout value..
			tmr_entry *entry = tmr_node( id );
			if( entry == NULL )
				return;
			list_lock();
			// we modify it 
			...
			entry->lt_fire = timeGetLongTime() + t_period;
			list_unlock();
			// but we do not insert again, just wake up the tiemr thread
			pthread_mutex_lock(&w_lock);
			pthread_cond_signal(&w_cond);
			pthread_mutex_unlock(&w_lock);
		}else{
			// this is the call back function not ready, or the count is 0, ignore.
		}
	}
	return -1;
}
